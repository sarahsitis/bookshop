...
class Auth extends CI_Controller
{
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'seirasofuto@gmail.com',
            'smtp_pass' => 'heisei10',
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
        ...
        $this->email->from('seirasofuto@gmail.com', 'Seira Sofuto');
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Your email ' . $email . ' has been activated. Please login.</div>');
                    redirect('auth');
                } else {
                    // delete user
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Account activation failed. Your token is expired.</div>');
                    redirect('auth');
                }
            } else {
